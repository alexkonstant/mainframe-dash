...
export default function MediaDeck({ audioData }) {
  ...
  return (
      <div className={`dashboard-panel ${theme === 'cyberpunk' ? 'cp-panel' : theme === 'material' ? 'md-panel' : ''}`} style={{ marginBottom: '20px' }}>
        ...
        {/* 1. HIDE DEFAULT HEADER FOR CLI THEME */}
        {theme !== 'cli' && (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', fontSize: '14px', fontFamily: 'var(--font)' }}>
              ...
              <div style={{ color: 'var(--accent)', marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button onClick={() => setIsExpanded(!isExpanded)} style={{ color: '#ffff55', cursor: 'pointer', background: 'transparent', border: 'none', padding: 0, fontSize: '14px', fontWeight: 'bold' }}>
                  {isExpanded ? '[ CLOSE_DIR ]' : '[ LS_PLAYLIST ]'}
                </button>
              </div>
              ...
              {/* Expandable Playlist Drawer in CLI Mode */}
              {isExpanded && (
                  <div style={{ marginTop: '15px', paddingTop: '10px', borderTop: '1px dashed #555' }}>
                    <div style={{ color: 'var(--accent)', marginBottom: '10px', display: 'flex', justifyContent: 'space-between' }}>
                      <span>root@mainframe:~# cat /var/lib/mpd/playlists/main.m3u</span>

                      {/* CLI File Upload Injection */}
                      <div>
                        <input type="file" ref={fileInputRef} onChange={handleFileUpload} accept="audio/*" style={{ display: 'none' }} id="cli-audio-upload" />
                        <label htmlFor="cli-audio-upload" style={{ color: '#00ff00', cursor: isUploading ? 'wait' : 'pointer', fontWeight: 'bold', opacity: isUploading ? 0.5 : 1 }}>
                          {isUploading ? '[ UPLOADING... ]' : '[ INJECT ]'}
                        </label>
                      </div>
                    </div>

                    <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                      {playlist.length > 0 ? playlist.map((trk, i) => {
                        const isPlaying = status.track === trk;
                        return (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', marginBottom: '4px', cursor: 'pointer' }} onClick={() => playIndex(i + 1)}>
                              <span style={{ width: '35px', color: isPlaying ? '#00ff00' : '#555' }}>[{String(i + 1).padStart(2, '0')}]</span>
                              <span style={{ flex: 1, color: isPlaying ? '#00ff00' : '#c0c0c0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                          {trk}
                                      </span>
                            </div>
                        );
                      }) : (
                          <div style={{ color: '#555', fontStyle: 'italic' }}>EOF: Playlist is empty.</div>
                      )}
                    </div>
                  </div>
              )}
