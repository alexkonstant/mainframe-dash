    ...
    return (
        <div className={`dashboard-panel network-radar ${theme === 'cyberpunk' ? 'cp-panel' : theme === 'material' ? 'md-panel' : theme === 'system7' ? 's7-panel' : ''}`}>
            ...
            <div className={theme === '90s' ? 'radar-content' : ''}>
                ...
                {radar ? (
                    <div className={theme === 'cyberpunk' ? 'cp-radar-grid' : theme === 'material' ? 'md-radar-grid' : ''} style={theme !== 'cyberpunk' && theme !== 'material' && theme !== 'y2k' && theme !== 'cli' ? { lineHeight: '1.6' } : {}}>
                        ...
                        {theme === 'system7' ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', fontSize: '14px', fontFamily: 'var(--font)' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    <div style={{ display: 'flex', borderBottom: '1px dashed #555', paddingBottom: '4px', marginBottom: '8px', color: '#888', fontSize: '12px' }}>
                                        <span style={{ flex: 1 }}>TARGET_HOST</span>
                                        <span style={{ width: '80px', textAlign: 'right' }}>STATE</span>
                                    </div>
                                    {radar.map((node, i) => (
                                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', color: '#c0c0c0', marginBottom: '4px' }}>
                                            <span style={{ flex: 1 }}>{node.name.toUpperCase()}</span>
                                            <span style={{
                                                width: '80px',
                                                textAlign: 'right',
                                                color: node.status === 'ONLINE' ? '#00ff00' : '#ff5555',
                                                fontWeight: 'bold'
                                            }}>
                                                {node.status === 'ONLINE' ? '[ UP ]' : '[DOWN]'}
                                            </span>
                            ...
                            /* Node Mapping Block (Existing Themes) */
                            radar.map((node, i) => (
                                theme === 'cyberpunk' ? (
                    ...
                    <div style={{ opacity: 0.5 }}>
                        {theme === '90s' ? 'Request timed out.' : theme === 'material' ? 'Scanning devices...' : theme === 'cli' ? 'root@mainframe:~# fping -g targets.lst\n> executing...' : '> AWAITING PING RETURN...'}
                    </div>
                )}
            </div>
        </div>
